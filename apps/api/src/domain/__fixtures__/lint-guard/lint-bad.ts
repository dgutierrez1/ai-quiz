...
// (`apps/api/tsconfig.json` `src/domain/__fixtures__/**`,
// `apps/api/src/domain/tsconfig.json` `__fixtures__/**`) and ESLint sets
...
//   2. Run `pnpm exec eslint apps/api/src/domain/__fixtures__/lint-guard/lint-bad.ts --no-ignore`.
//   3. Expected: errors — "domain MUST NOT import from @nestjs/* (spine AD-2)".
//   4. Run `pnpm exec eslint apps/api/src/domain/__fixtures__/lint-guard/lint-good.ts --no-ignore`.
