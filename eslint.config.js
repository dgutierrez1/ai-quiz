...
export default defineConfig(
  ...
  globalIgnores([
    ...
    '**/build/**',
    '**/.next/**',
    // Generated by `next build`; regenerated on every build and not ours to style.
    '**/next-env.d.ts',
    '**/out/**',
    '**/coverage/**',
    '**/playwright-report/**',
    ...
    'apps/api/test/fixtures/lint-guard/**',

    // Same arrangement for the `no-test-seams-in-src` fixtures: `seams-bad.ts`
    // deliberately contains the exact shapes the rule forbids, so it must not
    // fail the ordinary `pnpm lint:check`. `test/security/no-test-seams.test.ts`
    // lints it explicitly with `--no-ignore` and asserts it reports.
    'apps/api/test/fixtures/test-seams/**',

    // Story 5.3 ownership-rule fixtures — same deal. `bad.ts` is deliberately
    // an unscoped session query, so it must not fail the repo-wide gate; the
    // security test invokes eslint with `--no-ignore` and asserts a non-zero
    // exit, which is what actually proves the rule is live.
    'apps/api/test/security/__fixtures__/**',
  ]),
  ...
  // ── Base rules for all JS/TS source ────────────────────────────────────────
  {
    ...
    rules: {
      ...
      // Story 1.4 — dev-time half of the four-layer ownership model. Postgres
      // RLS is the runtime half; the constitution requires BOTH, so this must
      // stay 'error'. Turning it off re-opens the exact gap it exists to close.
      '@ai-quiz/local/no-unscoped-session-query': 'error',
      // Scoped to apps/api/src/** in a block below — test files legitimately
      // read NODE_ENV and define mock helpers, so it must NOT be global.
      '@ai-quiz/local/no-test-seams-in-src': 'off',
      // Still unimplemented — each rule lands with the code it guards.
  ...
  {
    files: ['apps/api/test/fixtures/lint-guard/**/*.ts'],
    languageOptions: {
      parserOptions: {
  ...
  // ── Tests may query session-scoped tables directly ─────────────────────────
  // `no-unscoped-session-query` guards PRODUCTION query paths. The security
  // suite's whole job is to issue deliberately unscoped queries and assert they
  // return zero rows under RLS — so enforcing the rule there would forbid the
  // very tests that prove the runtime half of the ownership model works.
  // Fixture directories are deliberately NOT exempted below — those files
  // exist precisely so a real ESLint run can prove the rule still fires.
  // Covers both `**/__fixtures__/**` (Story 5.3's own fixtures) and
  // `apps/api/test/fixtures/**` (the relocated Story 1.1/1.4 lint-guard
  // fixtures, which no longer sit under a `__fixtures__`-named directory).
  {
    files: ['apps/api/test/**/*.ts', 'apps/web/e2e/**/*.ts'],
    ignores: ['**/__fixtures__/**', 'apps/api/test/fixtures/**'],
    rules: {
      '@ai-quiz/local/no-unscoped-session-query': 'off',
    },
  },

  // ── No test seams in production source ─────────────────────────────────────
  // Production code must not branch on NODE_ENV==='test' or export mock
  // setters. Both shapes previously lived in the LLM and ingestion adapters,
  // where the "missing API key" fallback silently served fabricated quiz
  // content in production and the ingestion test branch bypassed the SSRF
  // guard entirely. Substitution now happens at the composition root via
  // AI_QUIZ_FAKE_ADAPTERS + @ai-quiz/test-doubles; this keeps it that way.
  // Scoped to src only — the fixture below proves the rule still fires.
  {
    files: ['apps/api/src/**/*.ts'],
    rules: {
      '@ai-quiz/local/no-test-seams-in-src': 'error',
    },
  },
  {
    files: ['apps/api/test/fixtures/test-seams/**/*.ts'],
    rules: {
      '@ai-quiz/local/no-test-seams-in-src': 'error',
    },
  },
  ...
  // Also applied to the relocated `apps/api/test/fixtures/lint-guard/**`
  // fixtures: `no-restricted-imports` matches on the literal import specifier,
  // not the importing file's real location, so the known-bad fixtures need
  // this rule explicitly re-applied here now that they no longer physically
  // live under `apps/api/src/domain/**`.
  {
    files: ['apps/api/src/domain/**/*.ts', 'apps/api/test/fixtures/lint-guard/**/*.ts'],
    rules: {
  ...
  // captures them).
  //
  // Story 1.3 EXTENDS the exception to the HealthController (driving/) so
  // its check-failure log line is visible during Story 1.3's lifetime —
  // pino lands in Story 1.6 and replaces console.* at the same call site.
  // Restricting this exception to driving/health/ (not all driving/) keeps
  // the AC #2 enforcement tight: every other driving-layer file still
  // fails lint on console.* and is forced to reach for a logger when one
  // exists.
  {
    files: ['apps/api/src/adapters/**/*.ts', 'apps/api/src/driving/health/**/*.ts'],
    rules: {
      'no-console': 'off',
    },
  },
